...
| JSON client | Ambiguous keys, trailing payload, context-path filter bypass | Unknown/duplicate fields and trailing values rejected; streamed body cap and context-relative filters | Proxy request limits and slow-client timeouts still require deployment configuration |
| Ordinary account | Scrape internal operational metrics | Prometheus requires a separately issued metrics.read scope; dangerous actuator routes denied | Trusted issuer and management-network isolation remain operator responsibilities |
...
| Worker crash/overlap | Spend another delivery's retry budget or leave partial results | Transaction-scoped coordinator lock, independent durable start, atomic final transaction and Kafka redelivery | Two DB connections per active delivery; parser stalls require isolation; long outages beyond Kafka retention need replay |
